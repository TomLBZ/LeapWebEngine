...
export class SDF_Generalfuncs{
    ...
    static sigmoid(x, oy, squeeze){
      return 1.0 / (1.0 + Math.exp(-x * squeeze)) - 0.5 + oy;//sigmoid function
    }
    static reflect3d(ray, norm){
      let n = VectorFunctions.vecNormalize(norm);
      let l = VectorFunctions.vecNormalize(ray);
      return VectorFunctions.vecMinus(VectorFunctions.vecScale(n,
        VectorFunctions.vecDot(n, l) * 2.0), l);
    }
}
export class SDF_Wrapper{
    constructor(){    }
    static sdfInfiniteRoundedCubes(p){//p is vec3
    ...
    static sdfTunnel(p){return Math.cos(p[0])+Math.cos(p[1]*1.5)+Math.cos(p[2])+Math.cos(p[1]*20.)*.05;}
    static sdfRibbon(p){
      let tmpvec1 = [Math.cos(p[2]*1.5)*.3,-.5+Math.cos(p[2])*.2,0.0];
      let tmpvec2 = [0.125,0.02,time+3.0];
      let tmpvec3 = [0.,0.,0.];
      let absdiff = VectorFunctions.vecAbs(VectorFunctions.vecMinus(p, tmpvec1));
      let tmpvec4 = VectorFunctions.vecMinus(absdiff, tmpvec2);
      let val = [Math.max(tmpvec4[0],tmpvec3[0]),Math.max(tmpvec4[1],tmpvec3[1]),Math.max(tmpvec4[2],tmpvec3[2])];
      return VectorFunctions.vecLength(val);
    }
